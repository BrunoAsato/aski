...
class Aski extends CI_Controller {
    ...
    public function index() {
        if((!$this->session->userdata('session_id')) && (!$this->session->userdata('logado'))){
            ...
            //die(print_r($session_id));
            redirect('aski/login');
        }

        //$this->data['ordens'] = $this->aski_model->getOsAbertas();
        $this->data['produtos'] = $this->aski_model->getProdutosMinimo();
        //$this->data['os'] = $this->aski_model->getOsEstatisticas();
        $this->data['estatisticas_financeiro'] = $this->aski_model->getEstatisticasFinanceiro();
        $this->data['menuPainel'] = 'Painel';
        $this->data['view'] = 'aski/painel';
    ...
    public function minhaConta() {
        ...
        $this->data['usuario'] = $this->aski_model->getById($this->session->all_userdata('id'));
        $this->data['view'] = 'aski/minhaConta';
        $this->load->view('tema/topo',  $this->data);
    ...
    public function alterarSenha() {
        ...
        $result = $this->aski_model->alterarSenha($senha,$oldSenha,$this->session->userdata('id'));
        if($result){
            $this->session->set_flashdata('success','Senha Alterada com sucesso!');
            redirect(base_url() . 'index.php/aski/minhaConta');
        }
        else{
    ...
    public function pesquisar() {
        ...
        $termo = implode('%', explode(' ', $termo));
        //die(print_r($termo));
        

        $data['results'] = $this->aski_model->pesquisar($termo);
        $this->data['produtos'] = $data['results']['produtos'];
        //$this->data['servicos'] = $data['results']['servicos'];
        //$this->data['os'] = $data['results']['os'];
        $this->data['clientes'] = $data['results']['clientes'];
        $this->data['view'] = 'aski/pesquisa';
        $this->load->view('tema/topo',  $this->data);
      
    }

    public function login() {
        $this->load->view('aski/login');
    }
    ...
    public function verificarLogin() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if($this->encryption->decrypt($usuario->senha) == $this->encryption->decrypt($senha)) {
                $dados = array('nome' => $usuario->nome, 'id' => $usuario->id_usuario,'permissao' => $usuario->id_permissao , 'logado' => TRUE);
    ...
    public function emitente() {   
        ...
        $data['dados'] = $this->aski_model->getEmitente();
        $data['view'] = 'aski/emitente';
    ...
    function do_upload(){
        ...
        if(!$this->permission->checkPermission($this->session->all_userdata('permissao'),'cEmitente')){
           $this->session->set_flashdata('error','Você não tem permissão para configurar emitente.');
           redirect(base_url());
        }
    ...
    public function cadastrarEmitente() {
        ...
        if ($this->form_validation->run() == false) {
            ...
            redirect(base_url().'index.php/aski/emitente');
            ...
            $retorno = $this->aski_model->addEmitente($nome, $cnpj, $ie, $logradouro, $numero, $bairro, $cidade, $uf,$telefone,$email, $logo);
            if($retorno){
    ...
    public function editarEmitente() {
        ...
        $this->form_validation->set_rules('nome','Razão Social','required|xss_clean|trim');
        $this->form_validation->set_rules('cnpj','CNPJ','required|xss_clean|trim');
        $this->form_validation->set_rules('ie','IE','urequired|xss_clean|trim');
        $this->form_validation->set_rules('logradouro','Logradouro','required|xss_clean|trim');
        $this->form_validation->set_rules('numero','Número','required|xss_clean|trim');
        $this->form_validation->set_rules('bairro','Bairro','required|xss_clean|trim');
        $this->form_validation->set_rules('cidade','Cidade','required|xss_clean|trim');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $retorno = $this->aski_model->editEmitente($id, $nome, $cnpj, $ie, $logradouro, $numero, $bairro, $cidade, $uf,$telefone,$email);
            if($retorno){
    ...
    public function editarLogo(){
        ...
        $retorno = $this->aski_model->editLogo($id, $logo);
        if($retorno){
